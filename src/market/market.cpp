...
bool Market::IsOpen()
{
	bool market_status = (time_ > add_minutes(30, mo_)) and (time_ < add_minutes(-30, mc_));
...
int Market::ToTicks(double price)
{
    ...
    double ub;
    auto it = pts_.begin();
	auto it_temp = pts_.begin();

	if (price < it->first) {
		throw std::invalid_argument("[Market] Invalid price " + std::to_string(price) + " for tick conversion.");
	}
	int c = 0;
    while (price+tick_size(it->first)/2.0 > it->first and it != pts_.end() and c++ < pts_.size() - 1) {
		it_temp = it;
		it_temp++;
		/*if (it_temp == pts_.end()) {
			std::cout << "pts inside ToTicks Method market.cpp has reached end" << std::endl;
			return ticks;
		}*/
...
double Market::ToPrice(int ticks)
{
	...
	if (ticks < it->first) {
		...
		std::cout << "Nagtive ticks : " << price << std::endl;
		return price;
	}
...
std::map<double, double> LondonStockExchange::pts_(std::string symbol)
{
    if (symbol == "AAL" or symbol == "BATS" or symbol == "GSK" or
        symbol == "VOD" or symbol == "HSBA" or symbol == "NVDA" or symbol == "APPL")
        return {
            {10000., 5},
            {5000., 1},
            {1000., 0.5},
