...
void Intraday<T1, T2>::DoAction(int action, float regime)
{
	...
	//outfile1.open("C:\\Users\\Abbas\\Desktop\\abbas\\rl_markets\\temporary\\regime.txt", ios::out | ios::app);
	int reg;
	if (regime >= 50.0)
...
double Intraday<T1, T2>::getVariable(Variable v)
{
    switch (v) {
		case Variable::m_sentiment:
			if ((double)f_bid_transactions.sum() > 0.0){
				double s_ratio = f_ask_transactions.sum() / (double)f_bid_transactions.sum();
				return s_ratio;
			}
			else
				return 0.5;
		//calculate ratio of sell(bearish mood) transactions volume to buy(bullish) 
