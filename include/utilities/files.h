...
vector<std::tuple<string, string, string>> get_single_file(
	string md_dir,
	string tas_dir,
	...
	vector<string> quote_files;
	vector<string> trade_files;
	for (auto& p : fs::recursive_directory_iterator(md_dir))
	{
		if (p.path().extension() == ".csv") {
			std::cout << p.path().string() << '\n';
			quote_files.push_back(p.path().string());
		}
	}

	for (auto& p : fs::recursive_directory_iterator(tas_dir))
	{
		if (p.path().extension() == ".csv") {
			std::cout << p.path().string() << '\n';
			trade_files.push_back(p.path().string());
		}
	}
	...
	for (auto s : symbols) {
		...
		if (i == quote_files.size() || i == trade_files.size())
			break;
		all_files.push_back(std::make_tuple(s, quote_files[i], trade_files[i]));
		i++;
	}
...
vector<std::tuple<string, string, string>> get_file_sample(
    vector<string> md_dir,
    vector<string> tas_dir,
    vector<string> symbols) {
	...
	vector<string> quote_files;
	vector<string> trade_files;
	for (vector<string>::iterator it = md_dir.begin(); it != md_dir.end(); it++) 
	{
	...
	for (vector<string>::iterator it = tas_dir.begin(); it != tas_dir.end(); it++) 
	{
		for (auto& p : fs::recursive_directory_iterator(*it))
		{
			if (p.path().extension() == ".csv") {
